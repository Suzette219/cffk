            ...
            <CardHeader><CardDescription>{{ messages.productCheckout.currentPrice }}</CardDescription><CardTitle class="text-3xl text-orange-600">¥{{ selectedSku.price }}</CardTitle></CardHeader><form class="grid min-w-0 gap-6" novalidate @submit.prevent="submit">
              <CardContent class="grid min-w-0 gap-4">
                ...
                <Field :data-invalid="Boolean(quantityError)">
                  <FieldLabel for="checkout-quantity"><span class="text-destructive">*</span> {{ messages.productCheckout.quantity }}</FieldLabel>
                  <Input id="checkout-quantity" v-model.number="quantity" type="number" :min="selectedSku.minBuy" :max="purchaseLimit" :step="1" :aria-invalid="Boolean(quantityError)" :aria-describedby="quantityError ? 'checkout-quantity-hint checkout-quantity-error' : 'checkout-quantity-hint'" required />
                  <div id="checkout-quantity-hint" class="grid gap-1 text-xs text-muted-foreground">
                    <span>{{ t(messages.productCheckout.quantityRange, { min: selectedSku.minBuy, max: selectedSku.maxBuy }) }}</span>
                    <span v-if="isStockLimited">{{ t(messages.productCheckout.availableStock, { count: availableStock }) }}</span>
                  </div>
                  <FieldError v-if="quantityError" id="checkout-quantity-error">{{ quantityError }}</FieldError>
                </Field>
                ...
                <div v-if="methods.length" class="grid gap-2 text-sm font-medium">
                  ...
                  <div class="grid grid-cols-2 gap-2" role="group" :aria-label="messages.productCheckout.paymentMethod">
                    ...
                    >
                      {{ item.provider === "ALIPAY" ? messages.productCheckout.paymentChannels.alipay : `${item.name}${item.channel ? `（${channelLabel(item.channel)}）` : ""}` }}
                    </Button>
                  </div>
                </div>
...
const quantityError = computed(() => {
  if (!Number.isSafeInteger(quantity.value) || quantity.value <= 0) return messages.value.productCheckout.quantityIntegerRequired;
  if (quantity.value < selectedSku.value.minBuy) return t(messages.value.productCheckout.quantityBelowMinimum, { min: selectedSku.value.minBuy });
  if (quantity.value > selectedSku.value.maxBuy) return t(messages.value.productCheckout.quantityAboveMaximum, { max: selectedSku.value.maxBuy });
  if (isOutOfStock.value || (isStockLimited.value && quantity.value > availableStock.value)) return messages.value.productCheckout.insufficientStock;
  return "";
});
function validateQuantity() {
  if (!quantityError.value) return true;
  toast.error(quantityError.value);
  document.getElementById("checkout-quantity")?.focus();
  return false;
}
...
function channelLabel(channel: string) { return ({ web: messages.value.productCheckout.paymentChannels.web, wap: messages.value.productCheckout.paymentChannels.wap, face_to_face: messages.value.productCheckout.paymentChannels.faceToFace, manual: messages.value.productCheckout.paymentChannels.manual, alipay: messages.value.productCheckout.paymentChannels.alipay, wxpay: messages.value.productCheckout.paymentChannels.wxpay } as Record<string, string>)[channel] ?? channel; }
</script>
