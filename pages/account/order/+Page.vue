      ...
      <div class="grid items-stretch gap-6 lg:grid-cols-[22rem_minmax(0,1fr)]">
        ...
        <section class="min-w-0 lg:flex" aria-live="polite">
          <Card v-if="result" class="h-full w-full">
            ...
            <CardFooter v-if="result.status === 'PENDING' && result.paymentStatus === 'UNPAID'" class="flex-wrap items-end justify-end gap-3 border-t pt-5"><OrderPaymentControls :key="result.orderNo" :order-no="result.orderNo" :created-at="result.createdAt" :proof-submitted="Boolean(result.paymentProof)" :disabled="resumingPayment" @busy="cancellingPayment = $event" @cancelled="refreshSelectedOrder" @refresh="refreshSelectedOrder" /><Button v-if="isFaceToFacePayment" :disabled="cancellingPayment" as-child><a :href="`/checkout?orderNo=${encodeURIComponent(result.orderNo)}`">{{ messages.accountOrders.payment.goToPayment }}</a></Button><Button v-else :disabled="resumingPayment || cancellingPayment" @click="resumePayment">{{ resumingPayment ? messages.accountOrders.payment.generating : messages.accountOrders.payment.resume }}</Button></CardFooter>
          </Card>
...
async function refreshSelectedOrder() {
  ...
  try {
    ...
    if (record && result.value?.orderNo === orderNo) {
      result.value = record;
      orders.value = orders.value.map((item) => item.orderNo === orderNo ? { ...item, status: record.status, paymentStatus: record.paymentStatus, deliveryStatus: record.deliveryStatus, paymentProofSubmitted: Boolean(record.paymentProof) } : item);
    }
