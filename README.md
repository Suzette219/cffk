...
收款码模式下，买家在支付页点击“我已完成付款”，填写支付宝交易订单号或上传付款成功截图（至少一项）。截图在浏览器压缩后存入订单凭证表，不依赖媒体存储配置，仅后台管理员可查看截图。

提交凭证不会直接确认支付或发货。后台订单管理点击“核实付款凭证”，核对支付宝实际到账后确认收款。未提交凭证的订单仍按 30 分钟超时关闭；已提交凭证的订单保留库存，暂停自动关闭和买家取消，由管理员确认收款或关闭处理。

升级本功能时，需先通过 `bun run db:migrate:remote` 应用 `0004_order_payment_proof.sql`，再发布新代码。此迁移只新增凭证表及索引，不修改历史订单。

### Bark 新订单推送

在 Cloudflare Worker 的“设置 → 变量和机密”中添加 Secret `BARK_DEVICE_KEY`，填入 Bark 地址 `https://api.day.app/<设备 Key>/...` 中的设备 Key。不要把真实 Key 提交到仓库。没有配置时不发送 Bark 通知。

订单创建成功即进入通知队列，现有每分钟定时任务推送“有订单”和订单号。发送失败会重试，不影响下单。该通知代表新订单，不代表支付到账；支付状态请以后台为准。已有订单不会补发，付款和发货不会重复触发此新订单通知。
