...
订单管理支持删除已关闭且未支付的订单。此操作从后台列表隐藏订单，保留历史交易、付款凭证及买家的订单查询。待支付订单需先关闭，已支付订单不允许删除。

用户管理支持删除普通用户。删除后列表隐藏该用户、禁止登录并撤销其全部会话；历史订单归属和邮箱占用保留，不能通过“启用用户”恢复。主管理员不能删除。

升级前先执行 `bun run db:migrate:remote` 应用 `0005_admin_soft_delete.sql`，再发布代码。该迁移只增加删除时间字段。
